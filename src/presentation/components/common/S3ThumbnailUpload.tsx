...
interface S3ThumbnailUploadProps {
...
export const S3ThumbnailUpload: React.FC<S3ThumbnailUploadProps> = ({
  onUploadSuccess,
  onUploadError,
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 리사이징 정보 로그 (개발용)
      if (data.reduction) {
        console.log(`썸네일 크기 감소: ${data.reduction}`);
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <p className="text-xs text-gray-500">
        이미지는 자동으로 800x600px 이하로 최적화됩니다.
      </p>
    </div>
  );
};