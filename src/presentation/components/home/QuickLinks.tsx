...
function getIcon(iconName?: string | null, title?: string): JSX.Element {
  ...
  if (title) {
    ...
    if (lowerTitle.includes('종교') || lowerTitle.includes('소득세')) return iconMap['document'];
    if (lowerTitle.includes('회계') || lowerTitle.includes('비영리')) return iconMap['calculator'];
    if (lowerTitle.includes('결산') || lowerTitle.includes('공시')) return iconMap['chart'];
    if (lowerTitle.includes('상담')) return iconMap['chat'];
    if (lowerTitle.includes('교육')) return iconMap['book'];
    if (lowerTitle.includes('법령') || lowerTitle.includes('법률')) return iconMap['scale'];
  }
...
export function QuickLinks({ links }: QuickLinksProps) {
  ...
  const displayLinks = links.length > 0 ? links.map((link, index) => ({
    id: link.id!,
    title: link.title,
