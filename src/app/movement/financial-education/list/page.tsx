    ...
    <div className="p-5">
      ...
      <div className="flex flex-wrap gap-2">
        {item.tags.map((tag, index) => (
          <span
            key={index}
            className={`px-3 py-1 text-xs font-medium rounded-full ${tagColors[tag] || 'bg-gray-100 text-gray-700'}`}
          >
            {tag}
          </span>
        ))}
    ...
    <div className="p-4">
      ...
      <div className="flex flex-wrap gap-1.5">
        {item.tags.map((tag, index) => (
          <span
            key={index}
            className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${tagColors[tag] || 'bg-gray-100 text-gray-700'}`}
          >
            {tag}
          </span>
        ))}
...
export default function EducationListPage() {
  const advancedEducations = educationData.filter(item => item.type === 'advanced');
  const feminismEducations = educationData.filter(item => item.type === 'feminism');
  const onlineEducations = educationData.filter(item => item.type === 'online');
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <section className="py-16 bg-gray-100">
        <div className="container mx-auto px-4">
          <div className="max-w-7xl mx-auto">
            <h2 className="text-3xl font-bold mb-8 text-center">온라인 강의 보기</h2>
