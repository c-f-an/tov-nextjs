...
export default function AdminReportsPage() {
  ...
  const [selectedType, setSelectedType] = useState<'all' | 'business' | 'finance'>('all');
            ...
            {reports.map((report) => (
              <tr key={report.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getTypeBadgeColor(report.type)}`}>
                ...
                <td className="px-6 py-4">
                  ...
                  {report.summary && (
                    <div className="text-sm text-gray-500 truncate max-w-md">{report.summary}</div>
                  )}
                </td>
