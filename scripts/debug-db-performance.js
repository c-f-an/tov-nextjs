...
async function debugDBPerformance() {
  console.log(
    `${colors.bright}${colors.cyan}=== TOV Database Performance Debugger ===${colors.reset}\n`
  );
  ...
  try {
    ...
    pool = mysql.createPool({
      host:
        ...
        "ptax-prod-rds.cnec2c6ca6bh.ap-northeast-2.rds.amazonaws.com",
    ...
    results.connection.status =
      connTime < 1000 ? "good" : connTime < 3000 ? "warning" : "bad";
    results.connection.message = `Connection established in ${connTime.toFixed(
      2
    )}ms`;

    console.log(
      getStatusIcon(results.connection.status) + results.connection.message
    );
    ...
    results.basicQuery.status =
      queryTime < 50 ? "good" : queryTime < 200 ? "warning" : "bad";
    results.basicQuery.message = `Simple query executed in ${queryTime.toFixed(
      2
    )}ms`;

    console.log(
      getStatusIcon(results.basicQuery.status) + results.basicQuery.message
    );
    ...
    const tableScanQueries = [
      ...
      {
        table: "posts",
        query: "EXPLAIN SELECT * FROM posts WHERE status = 'published'",
      },
      {
        table: "consultations",
        query: "EXPLAIN SELECT * FROM consultations WHERE status = 'pending'",
      },
      {
        table: "donations",
        query:
          "EXPLAIN SELECT * FROM donations WHERE created_at > DATE_SUB(NOW(), INTERVAL 7 DAY)",
      },
    ];
    ...
    results.tableScans.status =
      results.tableScans.issues.length === 0 ? "good" : "bad";
    if (results.tableScans.issues.length > 0) {
      console.log(
        `${colors.red}✗ Found ${results.tableScans.issues.length} potential table scans${colors.reset}`
      );
      results.tableScans.issues.forEach((issue) => {
        console.log(
          `  - ${issue.table}: ${issue.type} scan affecting ~${issue.rows} rows`
        );
      });
    } else {
      console.log(
        `${colors.green}✓ No table scan issues detected${colors.reset}`
      );
    }
    ...
    try {
      ...
      if (slowLog.length > 0) {
        console.log(
          `${colors.yellow}⚠ Found ${slowLog.length} slow queries${colors.reset}`
        );
        slowLog.forEach((q) => {
          console.log(
            `  - ${q.query_time}s: ${q.sql_text.substring(0, 80)}...`
          );
        });
      } else {
      ...
      console.log(
        `${colors.yellow}⚠ Cannot access slow query log (requires permissions)${colors.reset}`
      );
    }
    ...
    results.indexes.status =
      results.indexes.missing.length === 0 ? "good" : "warning";
    if (results.indexes.missing.length > 0) {
      console.log(
        `${colors.yellow}⚠ Missing indexes on ${results.indexes.missing.length} columns${colors.reset}`
      );
      ...
      console.log(
        `${colors.green}✓ All checked columns have indexes${colors.reset}`
      );
    }
    ...
    results.poolTest.status =
      results.poolTest.errors === 0 && poolTestTime < 1000 ? "good" : "warning";
    results.poolTest.message = `Executed ${concurrentQueries} concurrent queries in ${poolTestTime.toFixed(
      2
    )}ms with ${results.poolTest.errors} errors`;

    console.log(
      getStatusIcon(results.poolTest.status) + results.poolTest.message
    );
    ...
    console.log(
      `Connection Time: ${getStatusBadge(
        results.connection.status
      )} ${results.connection.time.toFixed(2)}ms`
    );
    console.log(
      `Query Latency: ${getStatusBadge(
        results.basicQuery.status
      )} ${results.basicQuery.time.toFixed(2)}ms`
    );
    console.log(
      `Table Scans: ${getStatusBadge(results.tableScans.status)} ${
        results.tableScans.issues.length
      } issues`
    );
    console.log(
      `Missing Indexes: ${getStatusBadge(results.indexes.status)} ${
        results.indexes.missing.length
      } columns`
    );
    console.log(
      `Pool Performance: ${getStatusBadge(results.poolTest.status)} ${
        results.poolTest.errors
      } errors`
    );
    ...
    if (results.connection.status !== "good") {
      recommendations.push({
        ...
        solution:
          "Check network latency between EC2 and RDS. Ensure they're in the same VPC/AZ.",
      });
    }
    ...
    if (results.tableScans.issues.length > 0) {
      recommendations.push({
        ...
        solution:
          ...
          results.tableScans.issues
            .map(
              (i) => `  ALTER TABLE ${i.table} ADD INDEX idx_status (status);`
            )
            .join("\n"),
      });
    }
    ...
    if (results.poolTest.errors > 0) {
      recommendations.push({
        ...
        solution:
          "Reduce connectionLimit or optimize query performance. Current limit may be too high for t2.micro.",
      });
    }
    ...
    recommendations.forEach((rec) => {
      console.log(
        `\n${getPriorityColor(rec.priority)}[${rec.priority}]${colors.reset} ${
          rec.issue
        }`
      );
    ...
    console.error(
      `\n${colors.red}Error during debugging:${colors.reset}`,
      error
    );
  } finally {
    if (pool) {
...
function getStatusBadge(status) {
  switch (status) {
    case "good":
      return `${colors.green}[GOOD]${colors.reset}`;
    case "warning":
      return `${colors.yellow}[WARN]${colors.reset}`;
    case "bad":
      return `${colors.red}[BAD]${colors.reset}`;
    default:
      return "[UNKNOWN]";
  }
}
